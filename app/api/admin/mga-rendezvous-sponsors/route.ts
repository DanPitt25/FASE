...
// GET - List all MGA Rendezvous sponsors
export async function GET(request: NextRequest) {
  ...
  try {
    const db = adminDb;
    const sponsorsSnapshot = await db.collection(COLLECTION_NAME).get();
...
// POST - Create a new MGA Rendezvous sponsor
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const docRef = await db.collection(COLLECTION_NAME).add(sponsorData);
    ...
    console.error('Error creating MGA Rendezvous sponsor:', error);
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}

// PATCH - Update an MGA Rendezvous sponsor
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    await db.collection(COLLECTION_NAME).doc(sponsorId).update(updateData);

    return NextResponse.json({ success: true });
  } catch (error: any) {
    console.error('Error updating MGA Rendezvous sponsor:', error);
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}

// DELETE - Delete an MGA Rendezvous sponsor
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const db = adminDb;
    await db.collection(COLLECTION_NAME).doc(sponsorId).delete();

    return NextResponse.json({ success: true });
  } catch (error: any) {
    console.error('Error deleting MGA Rendezvous sponsor:', error);
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}
