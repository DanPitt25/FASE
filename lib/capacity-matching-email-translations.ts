...
// Gender-specific greetings for each language (using first name style, not formal surname style)
export const greetingsByGender: Record<SupportedLanguage, Record<SalutationType, string>> = {
  ...
  de: { male: 'Lieber', female: 'Liebe', neutral: 'Liebe/r' },
  fr: { male: 'Cher', female: 'Chère', neutral: 'Cher/Chère' },
  es: { male: 'Estimado', female: 'Estimada', neutral: 'Estimado/a' },
  it: { male: 'Caro', female: 'Cara', neutral: 'Caro/a' },
  nl: { male: 'Beste', female: 'Beste', neutral: 'Beste' },
};
...
export const magicLinkEmailTranslations: Record<SupportedLanguage, MagicLinkEmailTranslations> = {
  en: {
    ...
    privacyNote: 'The privacy of your proprietary data is of paramount importance. The information you share will be used exclusively for the purpose of identifying potential matches with capacity providers attending the MGA Rendezvous. You will have full control over which organisations you wish to engage with, while retaining the full Digital Chain of Custody.',
  ...
  de: {
    ...
    requestInfo: 'Dazu würde ich es begrüßen, wenn Sie mir einige Informationen über Ihre Wachstumsambitionen für die nächsten drei Jahre mitteilen könnten. Sie können hier auf unseren kurzen Fragebogen zugreifen - das Ausfüllen sollte nicht mehr als fünf Minuten dauern. (Dieser Link funktioniert nur für Sie – wenn Sie möchten, dass ein Kollege diese Fragen beantwortet, teilen Sie uns bitte mit, wer, und wir werden ihn direkt kontaktieren.)',
  ...
  fr: {
    ...
    privacyNote: 'La confidentialité de vos données propriétaires est d\'une importance capitale. Les informations que vous partagez seront utilisées exclusivement dans le but d\'identifier des correspondances potentielles avec des fournisseurs de capacité participant au MGA Rendezvous. Vous garderez le contrôle total sur les organisations avec lesquelles vous souhaitez entrer en contact, tout en conservant la chaîne de custody numérique complète.',
  ...
  es: {
    ...
    privacyNote: 'La privacidad de sus datos propietarios es de suma importancia. La información que comparta se utilizará exclusivamente con el propósito de identificar posibles coincidencias con proveedores de capacidad que asistan al MGA Rendezvous. Usted tendrá control total sobre con qué organizaciones desea relacionarse, mientras mantiene la cadena de custodia digital completa.',
  ...
  nl: {
    ...
    requestInfo: 'Hiervoor zou ik het op prijs stellen als u wat informatie kunt delen over uw groeiambities voor de komende drie jaar. U kunt hier toegang krijgen tot onze zeer korte vragenlijst - het invullen zou niet meer dan vijf minuten moeten duren. (Deze link werkt alleen voor u – als u wilt dat een collega deze vragen beantwoordt, laat ons dan weten wie, en we nemen rechtstreeks contact met hen op.)',
...
// Generate only the editable body part of the email (up to and including "Best regards, William")
export function generateMagicLinkEmailBody(
  companyName: string,
  contactName: string,
  url: string,
...
// Generate the fixed footer part of the email (signature title, logo, FASE footer)
export function generateMagicLinkEmailFooter(language: SupportedLanguage = 'en'): string {
  const t = magicLinkEmailTranslations[language];
      ...
      <table cellpadding="0" cellspacing="0" border="0" style="margin-top: 24px;">
        <tr>
          <td style="padding-right: 16px; vertical-align: top;">
            <img src="https://fasemga.com/fase-logo-rgb.png" alt="FASE" style="height: 48px;" />
          </td>
          <td style="vertical-align: top;">
            <p style="color: #6b7280; font-size: 14px; line-height: 1.6; white-space: pre-line; margin: 0;">
              ${t.signatureTitle}
            </p>
          </td>
        </tr>
...
// Generate the full email HTML by combining body and footer (logo now in signature)
export function generateMagicLinkEmailHtml(
  ...
  const body = generateMagicLinkEmailBody(companyName, contactName, url, language, salutation);
  const footer = generateMagicLinkEmailFooter(language);
...
// Combine custom body with fixed footer for sending (logo is in footer/signature)
export function wrapEmailBodyWithTemplate(
  customBody: string,
  language: SupportedLanguage = 'en'
): string {
  const footer = generateMagicLinkEmailFooter(language);
