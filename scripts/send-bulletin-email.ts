...
async function sendBulletinEmail() {
  ...
  // Get locale and optional recipient from command line args
  const locale = (process.argv[2] || 'en') as SupportedLocale;
  const recipient = process.argv[3] || 'danielhpitt@gmail.com';
  const supportedLocales: SupportedLocale[] = ['en', 'de', 'fr', 'es', 'it', 'nl'];

  if (!supportedLocales.includes(locale)) {
    console.error(`Unsupported locale: ${locale}. Supported: ${supportedLocales.join(', ')}`);
    process.exit(1);
  }
  ...
  console.log(`Sending bulletin email in ${locale.toUpperCase()} to ${recipient}...`);
...
sendBulletinEmail().catch(console.error);
